...
def process_file(
    ...
    for location, measurements in sorted(result.items()):
        print(# 0 min, 1 max, 2 sum,3 count
            f"{location.decode('utf8')}= min:{measurements[0]:.1f} / median:{(measurements[2] / measurements[3]) if measurements[3] !=0 else 0:.1f} / max: {measurements[1]:.1f}",
            end="\n ",
        )
    print("\b\b} ")
